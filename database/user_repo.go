...
func CreateUser(user *models.User) error {
	stmt, err := DB.Prepare("INSERT INTO users(uuid, username, age, gender, first_name, last_name, email, password_hash) VALUES(?, ?, ?, ?, ?, ?, ?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()

	res, err := stmt.Exec(user.UUID, user.Username, user.Age, user.Gender, user.FirstName, user.LastName, user.Email, user.PasswordHash)
	if err != nil {
		return err
	}
